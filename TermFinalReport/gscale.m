...
%   argument.
%
%   G = GSCALE(F, 'full16') scales the intensitis of F to the full 16-bit
...
%   its values are not in th range [0, 1], then GSCALE scales it to this
%   range before processing.
%
%   The class of the output is the same aas the class of the input.
...
if strcmp(class(f), 'double') && (max(f(:)) > 1 || min(f(:)) < 0)
    % Force the input into a grayscale image with range [0, 1].
    f = mat2gray(f);    
end
